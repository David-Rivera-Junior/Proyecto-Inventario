...
<body class="container-fluid">
		 ...
		 <h1 class="text-center mt-fixed-nav">ELIMINAR USUARIO</h1>
         <c:url value="/UsuarioController" var="registerUrl" /> 
	         ...
	         <form method="POST" action="${registerUrl}" class="col-6 offset-3">
	                <c:forEach items="${datos}" var="usuario">
	                    ...
	                    <section class='alert alert-danger col-8 offset-2 text-center ' role='alert' id='alerta'>
	                        ¿Esta seguro de eliminar el Usuario: ${usuario.nombre}?
	                    </section>               
	                </c:forEach>
        ...
        <c:if test="${success != null}">
        	...
        	<a  class="btn btn-outline-success text-center col-4 offset-4" href="http://localhost:8080/ProyectoInventario/UsuarioController?action=list">Retornal a la lista principal</a>
      </c:if>
</body>
</html>